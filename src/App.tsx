...
// Новая универсальная функция для обработки ошибок
const handleApiError = (error: unknown, defaultMessage: string) => {
  console.error(defaultMessage, error);
  toast.error(defaultMessage); // Всплывающее уведомление с ошибкой
};
...
// Универсальная функция для загрузки данных
  ...
  const fetchReservedItems = async () => {
    try {
      ...
      handleApiError(error, 'Ошибка загрузки зарезервированных товаров.');
      setReservedItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const fetchSoldReservations = async () => {
    try {
      ...
      handleApiError(error, 'Ошибка при загрузке проданных товаров.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleScan = async (id: string) => {
    ...
    try {
      ...
      handleApiError(error, 'Ошибка при выполнении операции.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReservedItemScan = async (orderNumber: string) => {
    try {
      ...
      handleApiError(error, 'Не удалось обработать резерв.');
    } finally {
      setLoading(false);
    }
  };
                ...
                <ReservedItemsList
                    ...
                    onWeekFilter={async (week: string) => {
                      try {
                        ...
                        handleApiError(error, 'Не удалось загрузить зарезервированные товары.');
                        setReservedItems([]);
                      } finally {
                        setLoading(false);
                      }
