...
function App() {
  ...
  const handleScan = async (id: string) => {
    ...
    try {
      ...
      // Обработка получения QR-кода
      const qrResponse = await api.get(`/items/${id}/qrcode`, { responseType: 'arraybuffer' });
      const qrBlob = new Blob([qrResponse.data], { type: 'image/png' }); // ГенерацияBlob для QR-кода
      const qrUrl = window.URL.createObjectURL(qrBlob);

      // Создаем ссылку для скачивания файла
      const link = document.createElement('a');
      link.href = qrUrl;
      link.download = `qrcode-${id}.png`; // Присваиваем имя файла
      link.click();
