...
import React, { useState } from 'react';
import { Item } from '../types/Item';
import Accordion from './Accordion'; // Подключение компонента аккордеона
import '../styles/ReserveForm.css';
import { toast } from "react-toastify";
import 'react-toastify/dist/ReactToastify.css';
...
const ReserveForm: React.FC<ReserveFormProps> = ({
    ...
    return (
        <>
            ...
            {/* Добавление аккордеонов для отображения QR-кодов */}
            <div className="qr-accordion-list">
                {items.map((item) => (
                    <Accordion key={item.id} title={`QR для ${item.name}`}>
                        <p>Элемент: {item.name}</p>
                        <p>Доступное количество: {item.quantity}</p>
                        <img
                            src={`/path/to/qrcode/${item.id}`}
                            alt={`QR ${item.name}`}
                            className="qr-image"
                        />
                    </Accordion>
